...
public partial class StatisticsDialog : Dialog
{
	...
	protected override void OnShow()
	{
		...
		for (int i = 0; i < GetNetwork.GetStatisticsManager.GetCount(); i++)
		{
			Statistic statistic = GetNetwork.GetStatisticsManager.GetStatisticAtIndex(i);
			if (statistic != null && i < _Containers.Count)
			{
				StatisticControl statisticControl = _StatisticControls[i];
				if(statisticControl != null)
					statisticControl.Set(statistic);
			}
		}
	}
}
